...
class Enemy {
    constructor(x, y, type = 'basic') {
        ...
        switch (type) {
            ...
            case 'hive_boss':
                // 2스테이지 보스: 심연의 추적자
                this.speed = 80;
                this.maxHealth = 150; // 체력 하향 (400 -> 250 -> 150)
                this.damage = 1;
                this.color = '#330066';
            ...
            case 'stage3_boss':
                ...
                this.phaseThreshold = [80, 40]; // 페이즈 전환 체력 (120 기준)
                this.currentPhase = 1; // 1, 2, 3 페이즈

                // 전기 창 패턴 관련
                this.spearsFired = 0; // 발사된 창 개수
                this.spearCooldown = 0; // 창 발사 쿨다운
                break;
        }
    ...
    update(deltaTime, game) {
        ...
        // 소환 대기 중
        if (this.isSpawning) {
            // 첫 draw 호출 후에만 타이머 증가 (경고 표시가 보이도록)
    ...
    updateStage3BossAI(deltaTime, game) {
        ...
        if (this.patternTimer >= 3.0) {
            this.patternTimer = 0;
            this.currentPattern = (this.currentPattern + 1) % 2; // 0: 전기 창, 1: 원형 탄막
        ...
        switch (this.currentPattern) {
            case 0: // 전기 창 패턴
                // 총 4개: 1발, 1발, 2발 동시
                if (this.spearsFired < 4 && this.spearCooldown <= 0) {
                    const dx = this.target.x - this.x;
                    const dy = this.target.y - this.y;
                    const angle = Math.atan2(dy, dx);

                    if (this.spearsFired < 2) {
                        // 첫 2개는 한발씩
                        this.fireElectricSpear(game, angle);
                        this.spearsFired++;
                        this.spearCooldown = 0.8; // 0.8초 간격
                    } else if (this.spearsFired === 2) {
                        // 마지막 2개는 동시에
                        this.fireElectricSpear(game, angle - 0.2); // 약간 왼쪽
                        this.fireElectricSpear(game, angle + 0.2); // 약간 오른쪽
                        this.spearsFired += 2;
                        this.spearCooldown = 0.8;
                    }

                    game.spawnParticles(this.x, this.y, '#00FFFF', 15);
                }
                break;
            ...
            case 1: // 원형 탄막 (기존 유지)
                if (this.aiTimer >= 0.6) {
                    this.aiTimer = 0;
                    const bulletCount = 8 + this.currentPhase * 2;
                    for (let i = 0; i < bulletCount; i++) {
    ...
    fireElectricSpear(game, angle) {
        ...
        spear.isEnemy = true;
        spear.color = '#00FFFF'; // 시안색 (전기 느낌)
        spear.speed = 250; // 빠른 속도
